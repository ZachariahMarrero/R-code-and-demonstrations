...
#I can't think of any obvious, principled, way to do so.  And there may not be one based on Fisher's argument.  However, we might try to construct a prediction rule anyway. Perhaps his argument only applies to rules delimited by some special property that we're under no obligation to confine ourselves to.  One idea would be to simulate several populations with a wide range of variances and then using our md/md.scaled heuristic as the rule, and essentially trying to figure out if we can predict k. 

#For now, we do know that md.scaled should be sqrt(2/pi).  It doesn't end up being that though.  Not sure why. But also, if we use sqrt(2/pi) instead of md.scaled, our efficiency actually gets worse. It essentially reverts back to the -14%. see here: 
...
#SO, we know what the md.scaled 'should' be in principle and we know that setting it to that value results in worse performance...again, this is tied to the fact that to produce md.scaled, we had to first use standard deviation.... SO then, what if we just exaggerate our distance from that value.  if md.scaled is lower than expected, let's make it multiple times lower, etc. 
...
#Back up to -3.85% now.... so there's a limit at which increasing linearly stop helping. when I look at the curve, I can see that the curve is shifted slightly to the right. At 5, we're overestimating. Let's look at diagnositics. 
res4 <- as.data.frame(res3)
